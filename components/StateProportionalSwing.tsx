...
const StateProportionalSwing: React.FC<StateProportionalSwingProps> = ({
  ...
  const seatTableData: SeatTableEntry[] = barChartDataForColumn1.map((item) => {
        ...
        <ResponsiveContainer width="100%" height={250 + barChartDataForColumn1.length * 65}>
          ...
          >
            ...
            <Bar dataKey="currentPercent" name="% Atual (2022)" barSize={22} radius={[3, 3, 0, 0]}>
              {barChartDataForColumn1.map((entry, index) => (
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={180 + dataForSwingColumnChart.length * 55}>
            <BarChart
              ...
              layout="horizontal"
              margin={{ top: 40, right: 15, left: 15, bottom: 25 }}
              barCategoryGap="5%"
              barGap={0}
            >
              ...
              <Bar dataKey="swing" name="Swing" barSize={50}>
                {dataForSwingColumnChart.map((entry, index) => (
                ...
                <LabelList
                  ...
                  content={(props: any) => {
                    ...
                    if (value == null || !payload?.legend) return null;
        ...
        <div>
          ...
          <div className="space-y-1">
            ...
            {seatTableData.map((item) => {
              ...
              const seatSign = item.seatChange === 0 ? "" : item.seatChange > 0 ? "+" : "";
              const percentSign = item.swingPercent === 0 ? "" : item.swingPercent > 0 ? "+" : "";
              ...
              if (item.swingPercent > 0.05) percentChangeTagClasses = 'bg-green-100 text-green-700';
              else if (item.swingPercent < -0.05) percentChangeTagClasses = 'bg-red-100 text-red-700';
                ...
                <div key={item.legend} className="grid grid-cols-5 gap-1 items-center py-1.5 border-b border-gray-200 last:border-b-0 text-sm">
                  ...
                  <div className="text-right col-span-1">
                    <span className={`px-1.5 py-0.5 text-xs font-semibold rounded-full ${percentChangeTagClasses}`}>
                      {percentSign}{item.swingPercent.toFixed(1)}%
                    </span>
                  </div>
                </div>
              );
